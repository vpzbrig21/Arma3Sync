...
- The regular Windows release is the **Standard** package:
  `Arma3Sync-2026.2.5.zip` and `Arma3Sync-2026.2.5-setup.exe`. It includes a
...
- An optional **Compact** package is available as
  `Arma3Sync-2026.2.5-compact.zip` and
  `Arma3Sync-2026.2.5-compact-setup.exe`. It does not include a runtime and
...
- Centralized the application version at `version.properties`; this release uses `2026.2.5`.
...
- GitHub Releases are enabled by default in new installations. Arma3Sync
  passes the selected source explicitly to the updater for both checking and
  downloading; `-manifest` can bypass GitHub for an individual operation, and
  `github.enabled = false` remains an authoritative opt-out.
- The Preferences dialog includes a persistent “Prefer GitHub Releases for
  application updates” setting. Existing preference files default safely to
  GitHub preference, while users can switch back to the configured manifest
  source without editing the updater JAR.
