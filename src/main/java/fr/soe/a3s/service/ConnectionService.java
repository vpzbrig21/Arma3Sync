...
public class ConnectionService extends ObjectDTOtransformer {
	...
	private void init(int nbConnections, AbstractProtocole protocol) throws CheckException {
		...
		for (int i = 0; i < nbConnections; i++) {
			...
			} else if (protocol instanceof Webdav) {
				AbstractConnexionDAO webdavDAO = new WebdavDAO();
				connexionDAOPool.add(webdavDAO);
			} else {
	...
	public void getSync(String repositoryName) throws RepositoryException, IOException {
		...
		try {
			syncTreeDirectory = (SyncTreeDirectory) connexionDAOPool.get(0).downloadA3SObject(repositoryName,
					repository.getProtocol(), DataAccessConstants.SYNC_FILE_NAME);
		} finally {
	...
	public void getServerInfo(String repositoryName) throws RepositoryException, IOException {
		...
		try {
			serverInfo = (ServerInfo) connexionDAOPool.get(0).downloadA3SObject(repositoryName,
					repository.getProtocol(), DataAccessConstants.SERVERINFO_FILE_NAME);
		} finally {
	...
	public void getChangelogs(String repositoryName) throws RepositoryException, IOException {
		...
		try {
			changelogs = (Changelogs) connexionDAOPool.get(0).downloadA3SObject(repositoryName,
					repository.getProtocol(), DataAccessConstants.CHANGELOGS_FILE_NAME);
		} finally {
	...
	public void getAutoconfig(String repositoryName) throws RepositoryException, IOException {
		...
		try {
			autoConfig = (AutoConfig) connexionDAOPool.get(0).downloadA3SObject(repositoryName,
					repository.getProtocol(), DataAccessConstants.AUTOCONFIG_FILE_NAME);
		} finally {
	...
	public void getEvents(String repositoryName) throws RepositoryException, IOException {
		...
		Events events = null;
		try {
			events = (Events) connexionDAOPool.get(0).downloadA3SObject(repositoryName, repository.getProtocol(),
	...
	public void checkRepository(String repositoryName) throws RepositoryException, IOException {
		...
		System.out.println("Checking repository: " + repositoryName + " on url: "
		...
		ServerInfo previousServerInfo = repository.getServerInfo();
	...
	private boolean currentServerServerInfoChanged(ServerInfo previous, ServerInfo current) {
		if (previous.getRevision() != current.getRevision()) {
			return true;
		}
		if (previous.getNumberOfFiles() != current.getNumberOfFiles()) {
			return true;
		}
		if (previous.getTotalFilesSize() != current.getTotalFilesSize()) {
			return true;
		}
		return false;
	}
	...
	public AutoConfigDTO importAutoConfig(AbstractProtocole protocol) throws IOException {
		...
		System.out.println("Importing autoconfig from url: " + protocol.getProtocolType().getPrompt()
				+ protocol.getHostname() + ":" + protocol.getPort() + protocol.getRemotePath() + "/"
				+ DataAccessConstants.AUTOCONFIG_FILE_NAME);

		AutoConfigDTO autoConfigDTO = null;
		AutoConfig autoConfig = connexionDAOPool.get(0).importAutoConfig(protocol);
		if (autoConfig != null) {
	...
	public String CheckPartialFileTransfer(String repositoryName) throws RepositoryNotFoundException, IOException {
		...
		String header = connexionDAOPool.get(0).checkPartialFileTransfer(repository);
	...
	public void determineFilesCompletion(String repositoryName, SyncTreeDirectoryDTO parent)
		...
		boolean noPartialFileTransfer = repository.getServerInfo().isNoPartialFileTransfer();
		...
		ConnectionCompletionProcessor completionProcessor = new ConnectionCompletionProcessor(filesToCheck,
				connexionDAOPool, repository);
		completionProcessor.run();
	}

	/* Synchronize Files */

	public void synchronize(String repositoryName, List<SyncTreeNodeDTO> filesToDownload) throws RepositoryException {
		...
		System.out.println(
				"Downloading from repository: " + repositoryName + " on url: " + repository.getProtocol().getHostUrl());

		ConnectionDownloadProcessor downloadProcessor = new ConnectionDownloadProcessor(filesToDownload,
				connexionDAOPool, repository, unZipFlowProcessor);
		downloadProcessor.run();
	}

	/* Check Repository Synchronization */

	public List<Exception> checkRepositoryContent(String repositoryName) throws RepositoryException, IOException {
	...
	public void getSyncWithUploadProtocole(String repositoryName) throws RepositoryException, IOException {
		...
		try {
			syncTreeDirectory = (SyncTreeDirectory) connexionDAOPool.get(0).downloadA3SObject(repositoryName,
					repository.getUploadProtocole(), DataAccessConstants.SYNC_FILE_NAME);
		} finally {
			repository.setSync(syncTreeDirectory);
		}
	}

	public void uploadRepository(String repositoryName, List<SyncTreeNodeDTO> filesToCheck,
			List<SyncTreeNodeDTO> filesToUpload, List<SyncTreeNodeDTO> filesToDelete, int lastIndexFileUploaded)
			throws RepositoryException, IOException {
		...
		System.out.println("Uploading repository: " + repositoryName + " on url: "
	...
	public void upLoadEvents(String repositoryName) throws RepositoryException, IOException {
		...
		connexionDAOPool.get(0).uploadA3SObject(repository.getEvents(), repository.getUploadProtocole(),
	...
	public String checkForUpdates(boolean devMode, AbstractProtocole protocol)
		...
		String updateVersionName = connexionDAOPool.get(0).downloadXMLupdateFile(devMode, protocol);
	...
	public void unZip(String repositoryName, List<SyncTreeLeafDTO> downloadedFiles) throws RepositoryNotFoundException {
		...
		for (SyncTreeLeafDTO leaf : downloadedFiles) {
			File zipFile = new File(
					leaf.getDestinationPath() + "/" + leaf.getName() + DataAccessConstants.ZIP_EXTENSION);
			if (zipFile.exists()) {
	...
	public void deleteExtraLocalFiles(String repositoryName, List<SyncTreeNodeDTO> listFilesToDelete)
		...
		for (SyncTreeNodeDTO node : listFilesToDelete) {
			File file = new File(node.getDestinationPath() + "/" + node.getName());
			if (file.exists()) {
				if (file.isFile()) {
	...
	public void setMaximumClientDownloadSpeed(double maximumClientDownloadSpeed) {
		for (AbstractConnexionDAO connexionDAO : connexionDAOPool) {
			connexionDAO.setMaximumClientDownloadSpeed(maximumClientDownloadSpeed);
		}
	}
