...
public class SyncPanel extends JPanel implements UIConstants {
	...
	public SyncPanel(final Facade facade) {
		...
		splitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, addonRepositoriesPanel, eventsPanel);
		...
		vertBox1.add(splitPane);
		{
			addonRepositoriesPanel.setLayout(new BorderLayout());
			{
				...
				containerPanel1.setBorder(
						BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), "Addons Repositories"));
			...
			eventsPanel.setLayout(new BorderLayout());
			{
				...
				containerPanel2
						.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(), "Events"));
				...
				if (isModifying) {
					...
					try {
						if (index != -1 && !(index >= tableRepositories.getRowCount())) {
							...
							boolean notify = (Boolean) model.getValueAt(index, 1);
							boolean auto = (Boolean) model.getValueAt(index, 2);
							repositoryService.setRepositoryNotification(name, notify);
							repositoryService.setRepositoryAutoUpdate(name, auto);
							updateRepositoriesAndEvents();
							tableRepositories.setRowSelectionInterval(index, index);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
	...
	/**
	 * http://stackoverflow.com/questions/2311449/jsplitpane-splitting-50- precisely
	 */
	@Override
	...
	private void updateRepositoriesAndEvents() {
		...
		while (iter.hasNext()) {
			...
			RepositoryStatus status = repositoryService.getRepositorySyncStatus(repositoryDTO.getName());
		...
		int j = 0;
		for (Iterator<EventDTO> iter2 = mapEvents.keySet().iterator(); iter2.hasNext();) {
	...
	private void buttonNewPerformed() {
		...
		RepositoryEditionDialog repositoryEditPanel = new RepositoryEditionDialog(facade);
	...
	private void buttonEditPerformed() {
		...
		int index = tableRepositories.getSelectedRow();
		if (index == -1 || index > tableRepositories.getRowCount()) {
			JOptionPane.showMessageDialog(facade.getMainPanel(), "Please select a repository.", "Information",
		...
		if (repositoryName != null) {
			boolean closed = facade.getMainPanel().closeRepository(repositoryName, null, false);
			if (closed) {
				RepositoryEditionDialog repositoryEditPanel = new RepositoryEditionDialog(facade);
				repositoryEditPanel.init(repositoryName);
				repositoryEditPanel.setVisible(true);
			}
		}
	}
	...
	private void buttonConnectAsAdminPerformed() {
		...
		if (repositoryName != null) {
			ProgressConnectionAsAdminDialog progressConnectionAsAdminPanel = new ProgressConnectionAsAdminDialog(facade,
					repositoryName);
	...
	private void buttonConnectToRepositoryPerformed() {
		...
		String repositoryName = (String) model.getValueAt(index, 0);
		if (repositoryName != null) {
			RepositoryPanel repositoryPanel = facade.getMainPanel().openRepository(repositoryName, null, true, false);
		...
		if (index != -1 && !(index >= mapEvents.size())) {
			...
			for (Iterator<EventDTO> iter2 = mapEvents.keySet().iterator(); iter2.hasNext();) {
				...
				if (index == j) {
					...
					RepositoryPanel repositoryPanel = facade.getMainPanel().openRepository(repositoryName,
							eventDTO.getName(), true, false);
					if (repositoryPanel != null) {
						repositoryPanel.synchronize(repositoryName, eventDTO.getName());
					}
					break;
				}
				j++;
			...
			JOptionPane.showMessageDialog(facade.getMainPanel(), "Please select an event.", "Information",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private class MyTableModel extends AbstractTableModel {
		...
		private final String[] columnNames = { "Name", "Notify", "Auto", "Status" };
