	...
	public void init(final String sourceDirectoryPath, final String targetDirectoryPath) {
		...
		Thread t = new Thread(new Runnable() {
			...
			public void run() {
				try {
					Thread.sleep(1000);
					int nbBikeys = commonService.extractBikeys(sourceDirectoryPath, targetDirectoryPath);
					setVisible(false);
					if (canceled) {
						JOptionPane.showMessageDialog(facade.getMainPanel(), "Extraction canceled!",
								"Bikey extractor wizard", JOptionPane.INFORMATION_MESSAGE);
					} else {
						if (nbBikeys != 0) {
							JOptionPane.showMessageDialog(facade.getMainPanel(),
									nbBikeys + " " + "have been copied to target directory.", "Bikey extractor wizard",
									JOptionPane.INFORMATION_MESSAGE);
						} else {
							JOptionPane.showMessageDialog(facade.getMainPanel(), nbBikeys + " " + "have been found.",
					...
					e2.printStackTrace();
					setVisible(false);
					JOptionPane.showMessageDialog(facade.getMainPanel(), e2.getMessage(), "Bikey extractor wizard",
