...
- Fixed the graphical update path. After confirming an available update,
  Arma3Sync now starts the updater without console-only mode so the updater
  window and progress information are visible.
- Kept explicit console mode for command-line update operations.
